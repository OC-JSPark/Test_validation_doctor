...
def test_KIDSCREEN_은_1점이_Never(settings):
    options = settings.score_options_for("1단계 KIDSCREEN-10")

    assert options == KIDSCREEN_SCORE_OPTIONS
    assert options[0].startswith("Never")


def test_다른_척도는_기본_선택지를_쓴다(settings):
    """PHQ 계열은 설정된 기본 선택지를 그대로 쓴다 (.env 로 덮어쓸 수 있다)."""
    for scale in ("2단계 PHQ-2", "3단계 PHQ-A"):
        assert settings.score_options_for(scale) == settings.score_options
        assert settings.score_options_for(scale) != KIDSCREEN_SCORE_OPTIONS
...
def test_코드_기본값은_PHQ_채점_관례를_따른다():
    """PHQ 계열은 0점부터 시작한다 (KIDSCREEN-10 은 1점부터)."""
    assert DEFAULT_SCORE_OPTIONS[0].startswith("Not at all (0")
    assert KIDSCREEN_SCORE_OPTIONS[0].startswith("Never (1")


def test_척도명_접두사가_붙어도_매칭된다(settings):
    """'1단계 KIDSCREEN-10' 처럼 앞에 단계 표기가 붙어도 알아봐야 한다."""
    assert settings.score_options_for("KIDSCREEN-10") == KIDSCREEN_SCORE_OPTIONS
    assert settings.score_options_for("kidscreen") == KIDSCREEN_SCORE_OPTIONS
...
def test_대화엔진_stage_를_척도명으로_바꾼다(settings):
    assert settings.scale_for_stage("stress") == "1단계 KIDSCREEN-10"
...
def test_실제_세션에_stage_와_척도가_붙는다(session_conn):
    ...
    assert all(s.scale_stage == "1단계 KIDSCREEN-10" for s in stressed)
